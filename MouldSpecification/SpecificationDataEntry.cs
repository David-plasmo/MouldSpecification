    ...
    public partial class SpecificationDataEntry : Form
    {
        ...
        private void SpecificationDataEntry_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                //set up buttons for adding new rows to polymer and machine datagrids
        ...
        private void BindControls()
        {
            try
            {
                ...
                txtAdditivePC.DataBindings.Add(new Binding("Text", bsMBComp, "AdditivePC", false, DataSourceUpdateMode.OnPropertyChanged, null));
                txtMBPercent.DataBindings.Add(new Binding("Text", bsMBComp, "MBPercent", false, DataSourceUpdateMode.OnPropertyChanged, null));
                //bsMBComp.AddingNew += MBComp_AddingNew;
        ...
        private void CopyToNew()
        {
            try
            {
                ...
                //DataSet ds = new ProductDetailsDAL().SelectMAN_Item(curItemID, null);
                //DataTable newDT = ds.Tables[0];
                //DataRow newRow = newDT.Rows[0];
                ...
                //newRow.EndEdit();
                //DataTable existingDT = dsIMSpecificationForm.Tables["MAN_Items"];
                //existingDT.Rows.Add(newRow);
                ////existingDT.ImportRow(newRow);
                //bsManItems.EndEdit();
                ...
                //bsManItems.Position = bsManItems.Count - 1;
                //drv = (DataRowView)bsManItems.Current;
                //curItemID = (int)drv.Row["ItemID"];
                //need statements below to change row state  *** NEITHER WORKING  ***
                //existingDT.AcceptChanges();
                //existingDT.Rows[existingDT.Rows.Count - 1].SetAdded();
